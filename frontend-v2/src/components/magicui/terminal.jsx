...
export const Terminal = ({
  ...
  const content = (
    <div
      ref={containerRef}
      className={cn(
        "z-0 h-full w-full flex flex-col overflow-hidden",
        className
      )}
    >
      <div className="flex flex-row gap-x-2 px-4 pt-4 pb-2 shrink-0">
        <div className="h-2 w-2 rounded-full bg-zinc-700/50"></div>
        <div className="h-2 w-2 rounded-full bg-zinc-700/50"></div>
        <div className="h-2 w-2 rounded-full bg-zinc-700/50"></div>
      </div>
      <pre className="px-4 pb-4 flex-1 overflow-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        <code className="grid gap-y-1.5 text-[11px] text-zinc-400 font-mono leading-tight">{wrappedChildren}</code>
      </pre>
    </div>
  )
